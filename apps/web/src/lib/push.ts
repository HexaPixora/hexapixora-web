...
export async function enablePush(): Promise<{ ok: boolean; state: PushState }> {
  ...
  if (permission !== "granted") return { ok: false, state: permission === "denied" ? "denied" : "unsubscribed" };
  ...
  if (!key) return { ok: false, state: "unsubscribed" }; // VAPID not configured server-side
