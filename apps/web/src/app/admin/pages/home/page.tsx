...
import { GripVertical, Eye, EyeOff, Settings2, LayoutDashboard, X, ToggleLeft, ToggleRight, Plus, Trash2 } from "lucide-react";
...
export default function HomepageBuilderPage() {
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      ...
      const loadedSections = layoutRes?.data?.data?.sections ?? DEFAULT_SECTIONS;
      setSections(loadedSections);
      setSavedSnapshot(JSON.stringify(loadedSections));
      if (defaultsRes?.data?.data) setModuleDefaults(defaultsRes.data.data);
      setLoaded(true);
    });
  }, []);

  const isDirty = loaded && savedSnapshot !== JSON.stringify(sections);
  ...
  const removeSection = async (id: string) => {
    const ok = await confirm({
      title: "Remove module?",
      description: "This module will be removed from the homepage.",
      confirmText: "Remove",
      destructive: true,
    });
    if (ok) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto pb-12">
      <div className="sticky top-0 z-20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-background/90 backdrop-blur-md py-4 border-b">
        <div>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Button variant="outline" onClick={openPreview} className="bg-background shadow-sm">
